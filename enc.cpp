...
//Se detalla el bloque por el cual 
// se desarrollará la encripci{on}
struct bloques
{
	char b_bits[44];
};

//Se declara la cadena por la cual se 
//manejarán los bloques 
struct benc
{
	char b_bits[94];
};

//Se detalla el bloque de 
//valores que se encriptarán
struct bloque
{
	char b_bits[47];
};

//Se declara la cadena de datos 
// por la cual se trabajará
typedef struct opb
{
	char bit;
	int key;
...
//Escribirá el texto encriptado 
//en un archivo txt
void *writeTextToFile(void *block)
{
	struct benc *ps;
	ps = (struct benc *)block;

	ofstream writeTextToFile("Textoencriptado.txt", ios::app);
	...
	writeTextToFile << ps->b_bits << endl;
}


//Obtiene el valor en bit correspondiente para 
//cada cadena
...
//retorna el valor del bloque obtenido en la iteración 
...
int main(int argc, char *argv[])
{
	...
	while (texto >> x)
	{
		...
		if (f % 88 == 0)
		{
			...
			rc = pthread_create(&tid, &attr, writeTextToFile, (void *)&ben);
